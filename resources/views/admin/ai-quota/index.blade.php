...
@section('content')
    <div class="page-head">
        <div>
            ...
            <p>Kelola permintaan kuota AI yang dikirim warga melalui WhatsApp. Pembayaran tidak diproses di dalam SIRKEL.
            </p>
        </div>
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi"><span class="metric-label">Disetujui</span><strong>{{ number_format($approvedCount) }}</strong>
        </div>
        <div class="kpi"><span class="metric-label">Nominal request
                disetujui</span><strong>Rp{{ number_format($approvedRevenue, 0, ',', '.') }}</strong><small class="muted">Bukan
                pencatatan pembayaran di SIRKEL.</small></div>
    </div>
        ...
        <div class="cluster mb-16">
            @foreach(['pending' => 'Menunggu', 'approved' => 'Disetujui', 'rejected' => 'Ditolak', 'all' => 'Semua'] as $key => $label)
                <a class="btn btn-sm {{ $status === $key ? 'btn-primary' : '' }}"
                    href="{{ route('admin.ai-quota.index', ['status' => $key]) }}">{{ $label }}</a>
            @endforeach
        </div>
                ...
                <tbody>
                    @forelse($requests as $item)
                        <tr>
                            ...
                            <td><strong>{{ $item->user->name }}</strong><br><small
                                    class="muted">{{ $item->user->email }}</small></td>
                            <td>Foto +{{ $item->asset_intake_quantity }}×<br>Deskripsi
                                +{{ $item->condition_description_quantity }}×<br>Bulk +{{ $item->bulk_ai_quantity }} sesi</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="empty">Tidak ada permintaan pada status ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
