...
@section('content')
    ...
    <div class="kpi-grid">
        <div class="kpi"><span class="metric-label">Biaya bulan
                ini</span><strong>${{ number_format($totalCost, 4) }}</strong></div>
            ...
            <div class="table-wrap">
                <table>
                    ...
                    <tbody>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="empty">Belum ada penggunaan AI.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
                ...
                <div class="list-row" style="align-items:flex-start;gap:12px">
                    <div style="min-width:0;flex:1">
                        <div class="split" style="gap:8px;align-items:center">
                            ...
                            <span
                                class="badge {{ $r->status === 'failed' ? 'danger' : 'success' }}">{{ \App\Support\SirkelUi::label($r->status) }}</span>
                        </div>
                        <div class="text-sm muted">{{ $r->model }} · {{ $r->created_at->format('d M H:i') }} ·
                            {{ number_format((int) $r->latency_ms) }} ms</div>
                        <div class="text-sm muted">{{ number_format((int) $r->input_tokens) }} token masuk ·
                            {{ number_format((int) $r->output_tokens) }} token jawaban ·
                            ${{ number_format($r->estimated_cost_usd, 6) }}</div>
