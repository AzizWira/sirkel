...
@section('content')
    ...
    <div class="subnav mt-16">
        ...
        <a class="{{ $scope === 'history' ? 'active' : '' }}"
            href="{{ route('partner.requests.index', ['scope' => 'history']) }}">Riwayat</a>
    </div>
                ...
                @foreach($incomingTransfers as $transfer)
                    ...
                    <a class="transfer-inbox-row" href="{{ route('partner.transfers.show', $transfer) }}">
                        <div class="transfer-inbox-main">
                            ...
                            <div class="text-sm muted mt-4">{{ $transfer->asset->passport_code }} · Dari
                                {{ $transfer->fromPartner?->business_name ?? 'Mitra sebelumnya' }}</div>
                            <div class="text-sm mt-8"><strong>Layanan yang dibutuhkan:</strong>
                                {{ \App\Support\SirkelUi::label($transfer->required_capability) }}</div>
                            @if($sourceAssessment)
                                <div class="text-sm muted mt-4">Hasil pemeriksaan sebelumnya:
                                    {{ \App\Support\SirkelUi::label($sourceAssessment->result_path) }}</div>
                            @endif
                        </div>
                        <div class="transfer-inbox-action">
                            <span class="text-sm muted">{{ $transfer->requested_at?->format('d M Y H:i') }}</span>
                            <span
                                class="btn btn-sm {{ $transfer->status === 'pending' ? 'btn-primary' : '' }}">{{ $transfer->status === 'pending' ? 'Tinjau Pengalihan' : 'Lihat' }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="text-sm muted">
                                    {{ \App\Support\SirkelUi::label($handover->effectiveHandoverType()) }}</div>
                            </td>
                            <td>
                                ...
                                @else
                                    ...
                                    <div class="text-sm muted">Jarak perkiraan {{ number_format((float) $handover->distance_km, 1) }}
                                        km</div>
                                @endif
                            </td>
                            <td><span
                                    class="badge {{ $handover->status === 'completed' ? 'success' : '' }}">{{ \App\Support\SirkelUi::handoverStatus($handover->status) }}</span>
                            </td>
                            <td>
                                {{ $handover->requested_date?->format('d M Y') ?? 'Belum ditentukan' }}
                                @if($handover->requested_time_start)
                                    <div class="text-sm muted">
                                        {{ substr($handover->requested_time_start, 0, 5) }}{{ $handover->requested_time_end ? ' – ' . substr($handover->requested_time_end, 0, 5) : '' }}
                                    </div>
                                @endif
                            </td>
                            <td>
                                @if($handover->status === 'completed' && $hasActiveCustody)
                                    <a class="btn btn-sm btn-primary"
                                        href="{{ route('partner.assets.show', $handover->asset) }}">Tangani Barang</a>
                                @else
